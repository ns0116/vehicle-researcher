...
# キャッシュ付きラッパー関数（TTL: 1時間）
@st.cache_data(ttl=3600)
def _cached_rankings(market: str, month: str, energy_type: str, series_type: str, count: int):
    return get_market_rankings(
        market=market, month=month, energy_type=energy_type,
        series_type=series_type, count=count,
    )


@st.cache_data(ttl=3600)
def _cached_specs(market: str, car_ids: tuple):
    return get_market_specs(market, list(car_ids))
...
if page == "販売台数ランキング":
    ...
    if st.button("🔍 ランキングデータを取得"):
        with st.spinner(f"{market}のデータを取得中..."):
            df, is_estimated = _cached_rankings(
                market=market,
        ...
        if selected_rows:
            ...
            if car_ids:
                with st.spinner(f"{market}スペックデータを取得中..."):
                    specs_df = _cached_specs(market, tuple(car_ids))
        ...
        try:
            ...
            if not car_ids:
                ...
                with st.spinner(f"{len(car_ids)} 台のスペックを{market}データから取得中..."):
                    df = _cached_specs(market, tuple(car_ids))
