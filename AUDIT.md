...
| # | 優先度 | 項目 | 状態 | 備考 |
|---|---|---|---|---|
| 1 | 高 | CLAUDE.mdに「データソースの既知の制限」を要約追記 | ✅ 2026-09-24 | `b33eeb6`。日本2ヶ月のみ実績値／米国2025年通年固定／中国は非公開API |
| 2 | 中 | CLAUDE.mdに`pytest tests/`の実行コマンドを追記 | ⬜ 未対応 | README.mdにのみ記載 |
| 3 | 中 | クラウドでの非対話的な動作確認手順（headless起動＋curl）を追記 | ⬜ 未対応 | |
| 4 | 中 | 1時間キャッシュの挙動をCLAUDE.mdにも追記 | ⬜ 未対応 | README.md:57にのみ記載 |
| 5 | 低 | Pythonバージョン要件を明記 | ⬜ 未対応 | requirements.txt・CLAUDE.md・READMEいずれにも無し |

補足（改善提案外）: 監査本文では未指摘だったが、`.gitignore`で`CLAUDE.md`が除外され一度もコミットされていなかった。機密情報・ローカル絶対パスが無いことを確認の上、除外を解除し追跡対象化済み（`b33eeb6`, 2026-09-24）。
